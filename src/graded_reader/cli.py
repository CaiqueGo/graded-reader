...
from graded_reader import __version__, anki, config, library
from graded_reader import deck as deck_module
from graded_reader.lexicon import Band, CoverageReport, LexiconError, coverage
from graded_reader.library import ImportAction, ImportResult
from graded_reader.profile import DEFAULT_NEW_WORDS, ProfileError, render
from graded_reader.store import database, texts, words

app = typer.Typer(
    help="reader -- a graded English reader with its own flashcards.",
...
def version() -> None:
    ...
    console.print(f"graded-reader {__version__}")
...
def list_texts(
    ...
    if not rows:
        console.print("[yellow]library empty[/yellow] -- use 'reader import'.")
        return
    console.print(table)
