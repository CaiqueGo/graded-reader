...
from graded_reader.web import charts
...
def render(request: Request, name: str, **context: object) -> HTMLResponse:
    ...
    """
    from graded_reader.web.app import templates
...
def import_inbox(request: Request, session: SessionDep) -> HTMLResponse:
    """Drain the inbox. The button half of what `reader import` does."""
...
#: One adaptation at a time, for one reader. See graded_reader.jobs.
...
def start_adaptation(
    ...
    cannot talk its way into changing anything -- see graded_reader.sources.
    """
    try:
